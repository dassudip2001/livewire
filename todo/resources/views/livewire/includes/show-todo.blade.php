    <div wire:key="{{ $to->id }}"
        ...
        <div class="flex justify-between space-x-2">
            ...
            <div class="flex item-center">
                @if ($to->is_done)
                    <input wire:click="toggle({{ $to->id }})" type="checkbox" checked>
                @else
                    <input wire:click="toggle({{ $to->id }})" class="mr-2" type="checkbox">
                @endif
            ...
            <div wire:click="edit({{ $to->id }})" class="flex items-center space-x-2">
                ...
                <button wire:click="deleteTo({{ $to->id }})"
        ...
        <span class="text-xs text-gray-500"> {{ $to->created_at }} </span>
        <div class="mt-3 text-xs text-gray-700">

            @if ($editID == $to->id)
                <button wire:click="update({{ $to->id }})"
